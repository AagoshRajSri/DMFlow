...
function sanitizeError(err) {
  ...
  return msg.replace(/(mongodb(?:\+srv)?:\/\/)([^:@\s]+):([^@\s]+)@/g, '$1<user>:<redacted>@');
}
...
async function start() {
  ...
  if (!mongoUri) {
    console.error('FATAL: MONGODB_URI not set. Set MONGODB_URI in environment or backend/.env');
    process.exit(1);
  }
  ...
  await DMJob.updateMany({ status: 'processing' }, { $set: { status: 'queued' } });
  ...
  const workerInterval = setInterval(async () => {
    try { await processOne(client); } catch (err) { console.error('worker error', err); }
  }, pollMs);

  const reconInterval = setInterval(async () => {
    try { await reconciliationLoop(client); } catch (err) { console.error('recon error', err); }
  }, reconciliationMs);
