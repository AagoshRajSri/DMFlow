...
function createLimiter() {
  const reservoir = Number(process.env.PSEUDOGRAM_RATE_LIMIT || 10);
  ...
  console.log("PseudoGram limiter settings:", {
    reservoir,
...
async function handleSend(client, j) {
  ...
  try {
    ...
    if (resp.status === 429) {
      ...
      const wait = ra
        ? Number(ra) * 1000
        : Number(process.env.PSEUDOGRAM_RATE_INTERVAL_MS || 60000);
    ...
    if (resp.status === 202 || resp.status === 200 || resp.status === 201) {
      ...
      await DMJob.findByIdAndUpdate(j._id, {
        $set: { status: "accepted", dmId, attempts, updatedAt: new Date() },
      });
      updated = true;
      ...
      try {
        ...
        if (upsertRes && upsertRes.upsertedCount === 0) {
          console.log(
            "DUPLICATE BLOCKED (delivery exists)",
            j.ruleId.toString(),
            j.recipientUserId,
          );
        }
    ...
    if (resp.status >= 500 && resp.status < 600) {
      ...
      if (attempts >= maxRetries) {
        ...
        console.log(
          ...
          j._id.toString(),
          "nextAttemptAt=",
          new Date(Date.now() + next).toISOString(),
        );
      }
      return;
    }
