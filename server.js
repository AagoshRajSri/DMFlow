...
const MONGODB_URI =
  process.env.MONGODB_URI || "mongodb://127.0.0.1:27017/linkplease";
const PSEUDOGRAM_API_KEY = process.env.PSEUDOGRAM_API_KEY || "";
if (!PSEUDOGRAM_API_KEY) {
  console.warn(
    "Warning: PSEUDOGRAM_API_KEY is not set. DM sends will fail without it.",
  );
}
...
async function handleWebhookJob(db, payload) {
  ...
  for (const rule of rules) {
    try {
      ...
      const existing = await dmsColl.findOne({
        user_id: userId,
      ...
      limiter
        .schedule(() => attemptSend(db, dmRecordId))
        .catch((err) => {
...
async function attemptSend(db, dmRecordId) {
  ...
  while (attempt < maxRetries) {
    ...
    try {
      await dmsColl.updateOne(
        { _id: dm._id },
        { $set: { attempts: attempt, updated_at: new Date() } },
      );
      ...
      if (resp.status === 201 || resp.status === 200) {
        ...
        await dmsColl.updateOne(
          { _id: dm._id },
          { $set: { dm_id, status: "queued", updated_at: new Date() } },
        );
        return;
      }
  ...
  for (const dm of toCheck) {
    ...
    try {
      const resp = await fetch(
        `${PSEUDO_BASE}/v1/dm/${encodeURIComponent(dm.dm_id)}`,
        {
          method: "GET",
      ...
      if (resp.status === 200) {
        ...
        await dmsColl.updateOne(
          { _id: dm._id },
          { $set: { status: mapped, updated_at: new Date(), remote: data } },
        );
...
async function main() {
  ...
  await db
    .collection("rules")
    .createIndex({ keyword: 1 })
    .catch(() => {});
  await db
    .collection("dms")
    .createIndex({ user_id: 1, rule_id: 1 })
    .catch(() => {});
  ...
  app.post("/rules", async (req, res) => {
    try {
      ...
      if (!keyword || !dm_message)
        return res
          .status(400)
          .json({ error: "keyword and dm_message required" });
      const rulesColl = db.collection("rules");
      const r = await rulesColl.insertOne({
        keyword,
        dm_message,
        created_at: new Date(),
      });
  ...
  app.get("/stats", async (req, res) => {
    try {
      ...
      const queued = await dmsColl.countDocuments({
        status: { $in: ["queued", "sending", "pending"] },
      });
      const duplicates_blocked = await dmsColl.countDocuments({
        status: "blocked",
      });
