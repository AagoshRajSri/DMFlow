...
async function main() {
  ...
  const dupGroups = await DMJob.aggregate([
    ...
    {
      $group: {
        _id: { ruleId: "$ruleId", recipientUserId: "$recipientUserId" },
        count: { $sum: 1 },
        docs: { $push: "$_id" },
      },
    },
  ...
  const migratedCount = await DMJob.countDocuments({
  ...
  console.log(
    "uniqueActiveGroups=",
    (activeUnique[0] && activeUnique[0].uniqueActive) || 0,
  );

  // 4) total active DMJobs
  const totalActive = await DMJob.countDocuments({
  ...
  console.log(
    "duplicateActiveIdempotencyKeys=",
    (dupIdemp[0] && dupIdemp[0].dupIdempotencyKeys) || 0,
  );
  ...
  const hasPartial = indexes.some(
    (ix) =>
      ix.key &&
      ix.key.idempotencyKey &&
      ix.partialFilterExpression &&
      ix.partialFilterExpression.status &&
      ix.partialFilterExpression.status.$in,
  );
