...
async function sendBatch(url, count, concurrency = 50) {
  ...
  for (let i = 0; i < count; i++) {
    const payload = {
      ...
      data: { comment_id: `c_${i}`, text: 'PRICE please', created_at: new Date().toISOString(), from: { user_id: `u_${i % 10}` } }
    };
    ...
    tasks.push(() => (async () => {
      try {
        ...
        await axios.post(url + '/webhook', raw, { headers: { 'Content-Type': 'application/json', 'X-PseudoGram-Signature': 'sha256='+sig } });
      } catch (e) {}
    })());
  }
...
(async () => {
  ...
  await wait(20000);
  try {
    const s = await axios.get(url + '/stats').then(r=>r.data).catch(()=>null);
