...
test("429 respects Retry-After header", async () => {
  const job = await DMJob.create({
    ruleId: "000000000000000000000002",
    recipientUserId: "u2",
    message: "hi",
    status: "queued",
    nextAttemptAt: new Date(0),
...
test("400 does not retry and marks failed", async () => {
  const job = await DMJob.create({
    ruleId: "000000000000000000000003",
    recipientUserId: "u3",
    message: "hi",
    status: "queued",
    nextAttemptAt: new Date(0),
...
test("202 accepted stores dmId and status accepted, not delivered", async () => {
  const job = await DMJob.create({
    ruleId: "000000000000000000000004",
    recipientUserId: "u4",
    message: "hi",
    status: "queued",
    nextAttemptAt: new Date(0),
  ...
  const client = {
    sendDM: jest
      ...
      .mockResolvedValue({
        status: 202,
        data: { dm_id: "dm_123", status: "queued" },
        headers: {},
      }),
  };
  await processOne(client);
...
test("accepted -> delivered reconciliation updates job and delivery", async () => {
  const job = await DMJob.create({
    ruleId: "000000000000000000000005",
    recipientUserId: "u5",
    message: "hi",
    status: "accepted",
    dmId: "dm_ok",
  ...
  const client = {
    getDMStatus: jest
      .fn()
      .mockResolvedValue({ status: 200, data: { status: "delivered" } }),
  };
...
test("accepted -> failed reconciliation schedules retry", async () => {
  const job = await DMJob.create({
    ruleId: "000000000000000000000006",
    recipientUserId: "u6",
    message: "hi",
    status: "accepted",
    dmId: "dm_fail",
  ...
  const client = {
    getDMStatus: jest
      .fn()
      .mockResolvedValue({ status: 200, data: { status: "failed" } }),
  };
  await reconciliationLoop(client);
  const refreshed = await DMJob.findById(job._id);
...
test("failed reconciliation triggers retry until MAX_DM_RETRIES then failed", async () => {
  const job = await DMJob.create({
    ruleId: "000000000000000000000007",
    recipientUserId: "u7",
    message: "hi",
    status: "queued",
    attempts: 3,
  ...
  const client = {
    sendDM: jest.fn().mockResolvedValue({ status: 500, data: {}, headers: {} }),
  };
  await processOne(client);
  const refreshed = await DMJob.findById(job._id);
  expect(refreshed.status).toBe("failed");
...
test("idempotency key remains stable across retries", async () => {
  const job = await DMJob.create({
    ruleId: "000000000000000000000008",
    recipientUserId: "u8",
    message: "hi",
    status: "queued",
    attempts: 0,
