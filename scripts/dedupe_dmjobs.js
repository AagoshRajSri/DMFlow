...
async function main() {
  ...
  for (const [key, docs] of groups.entries()) {
    ...
    await DMJob.updateMany({ _id: { $in: ids } }, { $set: { status: 'failed', lastError: 'duplicate_migrated', updatedAt: new Date() } });
    fixed += toFail.length;
    console.log(`Group ${key} kept ${keep._id.toString()} failed ${toFail.length}`);
  }
